...
> **重要提示**: RecuRun 用性能换安全。它比普通递归慢,但能防止栈溢出。
...
| 测试用例 | 深度 | 普通递归 | RecuRun | 性能差距 |
|---------|------|----------|---------|----------|
| **斐波那契** | 30 | 4.6 ms ✅ | 77.4 ms ✅ | **16.9x** ⚠️ |
| **阶乘** | 1,000 | 0.15 ms ✅ | 0.44 ms ✅ | **2.9x** ⚠️ |
| **尾递归阶乘** | 5,000 | 0.71 ms ✅ | 0.74 ms ✅ | **1.0x** ⚠️ |
| **数组求和** | 5,000 元素 | 0.31 ms ✅ | 1.31 ms ✅ | **4.2x** ⚠️ |
| **深度递归** | 5,000 | 0.11 ms ✅ | 0.67 ms ✅ | **6.1x** ⚠️ |
| **极深递归** | 100,000 | ❌ 栈溢出 | 8.29 ms ✅ | **∞** ✅ |

### 核心结论

- **小规模递归 (< 1,000)**: RecuRun 比**普通递归慢 3-17 倍**
- **中等规模递归 (1,000-10,000)**: RecuRun 比**普通递归慢 1-6 倍**
- **深度递归 (> 10,000)**: 普通递归**栈溢出**,RecuRun **正常运行**
- **极深递归 (> 50,000)**: **只有 RecuRun 能完成任务**
...
|------|------|------|
| 性能关键代码 | ❌ 使用普通递归 | 执行更快 |
| 浅层递归 (< 1,000) | ❌ 使用普通递归 | 无栈溢出风险,更快 |
| 深度递归 (> 10,000) | ✅ 使用 RecuRun | 防止栈溢出 |
| 已有栈溢出问题 | ✅ 使用 RecuRun | 代码改动最小 |
| 异步递归 | ✅ 使用 RecuRun | 原生 async/await 支持 |
| 生产环境稳定性 | ✅ 使用 RecuRun | 可预测,不会崩溃 |

> **测试环境**: Node.js v24, 性能因机器和工作负载而异
